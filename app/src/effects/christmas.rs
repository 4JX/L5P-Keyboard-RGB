		...
		while !manager.stop_signals.manager_stop_signal.load(Ordering::SeqCst) {
			...
			match subeffect {
				...
				1 => {
					...
					for _i in 0..4 {
						manager.keyboard.solid_set_colors_to(used_colors_1);
						thread::sleep(Duration::from_millis(400));
						manager.keyboard.solid_set_colors_to(used_colors_2);
						thread::sleep(Duration::from_millis(400));
					}
				}
				2 => {
					let steps = 100;
					manager.keyboard.transition_colors_to(&[0; 12], steps, 1);
					...
					if left_or_right == 0 {
						for color in xmas_color_array {
							for j in (0..12).step_by(3) {
								...
								manager.keyboard.transition_colors_to(&used_colors_array, steps, 1);
							}
							for j in (0..12).step_by(3) {
				...
				3 => {
					...
					let steps = 30;
					for _i in 0..4 {
						manager.keyboard.transition_colors_to(&state1, steps, 1);
						thread::sleep(Duration::from_millis(400));
						manager.keyboard.transition_colors_to(&state2, steps, 1);
						thread::sleep(Duration::from_millis(400));
					}
				}
