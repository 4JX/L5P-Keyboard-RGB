...
impl ColorTile {
	...
	pub fn get_values(&mut self) -> [u8; 3] {
		...
		if !self.toggle_button.is_toggled() {
			values[0] = self.red_input.value().parse::<u8>().unwrap_or(0);
			values[1] = self.green_input.value().parse::<u8>().unwrap_or(0);
			values[2] = self.blue_input.value().parse::<u8>().unwrap_or(0);
		};
		values
	}
}
...
impl ZoneColorTiles {
	...
	pub fn get_values(&mut self) -> [u8; 12] {
		...
		if !self.left.toggle_button.is_toggled() {
			values[0] = self.left.red_input.value().parse::<u8>().unwrap_or(0);
			values[1] = self.left.green_input.value().parse::<u8>().unwrap_or(0);
			values[2] = self.left.blue_input.value().parse::<u8>().unwrap_or(0);
		};
		if !self.center_left.toggle_button.is_toggled() {
			values[3] = self.center_left.red_input.value().parse::<u8>().unwrap_or(0);
			values[4] = self.center_left.green_input.value().parse::<u8>().unwrap_or(0);
			values[5] = self.center_left.blue_input.value().parse::<u8>().unwrap_or(0);
		};
		if !self.center_right.toggle_button.is_toggled() {
			values[6] = self.center_right.red_input.value().parse::<u8>().unwrap_or(0);
			values[7] = self.center_right.green_input.value().parse::<u8>().unwrap_or(0);
			values[8] = self.center_right.blue_input.value().parse::<u8>().unwrap_or(0);
		};
		if !self.right.toggle_button.is_toggled() {
			values[9] = self.right.red_input.value().parse::<u8>().unwrap_or(0);
			values[10] = self.right.green_input.value().parse::<u8>().unwrap_or(0);
			values[11] = self.right.blue_input.value().parse::<u8>().unwrap_or(0);
		};
		values
	}
}
